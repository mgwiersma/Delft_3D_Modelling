...
inline float vec4::length2() const {
	...
	return float(m_data[0] * m_data[0] + m_data[1] * m_data[1] + m_data[2] * m_data[2] + m_data[3] * m_data[3]);
}
...
inline void vec4::normalize() {
	...
	if (magnitude != 0)
	{
		m_data[0] = m_data[0] / magnitude;
		m_data[1] = m_data[1] / magnitude;
		m_data[2] = m_data[2] / magnitude;
		m_data[3] = m_data[3] / magnitude;
	}
}
...
inline vec4 vec4::cross(const vec4& other) const {
	...
	return vec4(m_data[1] * other(2) - m_data[2] * other(1),
...
inline vec4 vec4::operator*(float scalar) {
	...
	return vec4(m_data[0] * scalar, m_data[1] * scalar, m_data[2] * scalar, m_data[3] * scalar);
}
